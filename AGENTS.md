...
- `pkg/task`, `pkg/workspace`, `pkg/metrics` are leaves (stdlib only)
- `pkg/sandbox` decorates `workspace` with a default-deny policy (areas + spawn allowlist)
...
- `pkg/sandbox` is a security boundary. Any change near it needs tests proving confinement still holds (traversal, sibling prefixes, list-leak, spawn gating). Default is always deny.
...
- **New tool**: implement `tool.Tool` (or `tool.Func`), register it, and grant it a `sandbox.Policy` in the `ServiceSpec`; no policy means no workspace access and no spawning.
